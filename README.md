# 👨‍🎓 About Me

- 👋 Hi! I'm **ChengAo Shen (沈骋骜)**, an AI researcher who enjoys turning ideas into software people actually use.
- 🎓 I'm a Ph.D. student in Computer Science at the [University of Houston](https://uh.edu/), working on **time series analysis** and **LLM-based agent systems**: lightweight forecasters that learn from large pre-trained models, and multi-agent systems that automate analytical workflows reliably. My work has appeared at **EMNLP, NeurIPS, ACL and IJCAI**.
- 🐱 I founded [CatVinci Studio](https://catvinci.com) to explore what agents can do for modern office work. Our most mature project is [Levis](https://github.com/CatVinci-Studio/Levis), an open-source Markdown editor with AI built in.
- 💬 Always happy to chat about time series, agents, or anything in between!

## 🤝 Let's Connect

- 🌐 Homepage: [chengaoshen.com](https://chengaoshen.com)
- 📫 Email: [chengao_shen@ieee.org](mailto:chengao_shen@ieee.org)

[![Google Scholar](https://img.shields.io/badge/-ChengAo%20Shen-4285F4?style=flat&logo=googlescholar&logoColor=white)](https://scholar.google.com/citations?user=aupSEN4AAAAJ&hl=en)
[![LinkedIn](https://img.shields.io/badge/-chengao--shen-0A66C2?style=flat&logo=data:image/svg%2bxml;base64,)](https://www.linkedin.com/in/chengao-shen)
[![X](https://img.shields.io/badge/-%40chengao__shen-000000?style=flat&logo=x&logoColor=white)](https://x.com/chengao_shen)
[![Hugging Face](https://img.shields.io/badge/-ChengAoShen-FFD21E?style=flat&logo=huggingface&logoColor=black)](https://huggingface.co/ChengAoShen)
